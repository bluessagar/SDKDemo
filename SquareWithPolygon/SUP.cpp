...
//	HANDLE hComm;  // Handle to the Serial port -- make this global so that it can be used inside the task as well
	...
	case WM_PAINT:
		...
		for (j = 0; j < VerticesOfPolygon-1; j++)
		{
			Polygon2Coordinates[j].x = (PolygonCoordinates[j].x + PolygonCoordinates[j+1].x)/2;
			Polygon2Coordinates[j].y = (PolygonCoordinates[j].y + PolygonCoordinates[j + 1].y)/2;
		}
		// Last co-ordinate
		Polygon2Coordinates[j].x = (PolygonCoordinates[j].x + PolygonCoordinates[0].x) / 2;
		Polygon2Coordinates[j].y = (PolygonCoordinates[j].y + PolygonCoordinates[0].y) / 2;

		Polygon(hDC, (const POINT *)PolygonCoordinates, VerticesOfPolygon);

		SelectObject(hDC, GetStockObject(DC_BRUSH));
		SetDCBrushColor(hDC, RGB(255, 255, 103));			// red

		Polygon(hDC, (const POINT *)Polygon2Coordinates, VerticesOfPolygon);
